...
$(document).ready(function() {
   var $adobeMenu = new adobeMenu($('.orig-menu'));
         ...
         switch(e.which) {
            ...
            case keys.left: {
               ...
               if (btnNdx < thisObj.$menuBtns.length-1) {
                  ...
                  $nextBtn = thisObj.$menuBtns.eq(btnNdx+1);
...
function adobeMenu($menu) {

   var thisObj = this;

   this.$navs = $menu.find('.submenu-wrapper');
   this.$menuBtns = $menu.find('[role=button]');
   this.bMobileView = false;
...
adobeMenu.prototype.toggleMenu = function($btn) {
   if ($btn.attr('aria-expanded') == 'false') {
      // hide all the menus
      this.$menuBtns.attr('aria-expanded', 'false');
      this.$navs.attr('aria-hidden', 'true');
